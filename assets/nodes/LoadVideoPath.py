...
def cv_frame_generator(video, force_rate, frame_load_cap, start_at_frame,
                       select_every_nth, meta_batch=None, unique_id=None):
    video_cap = cv2.VideoCapture(video)
    if not video_cap.isOpened() or not video_cap.grab():
        raise ValueError(f"{video} could not be loaded with cv.")

    # --- NEW LOGIC: Jump to the start_at_frame index ---
    if start_at_frame > 0:
        video_cap.set(cv2.CAP_PROP_POS_FRAMES, start_at_frame)
    ...
    # --- OLD SKIPPING LOGIC IS NO LONGER NEEDED ---
    # total_frame_count = 0
...
def load_video(meta_batch=None, unique_id=None, memory_limit_mb=None, vae=None,
    ...
    if 'start_time' in kwargs:
        ...
        start_time = kwargs['start_at_frame'] * target_frame_time
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "start_at_frame": ("INT", {"default": 0, "min": 0, "max": BIGMAX, "step": 1}),
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "LoadVideoPath": "Load Video Path (Creepybits)",
}
